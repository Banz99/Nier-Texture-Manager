...
namespace Lzo64
{
    ...
    public class LZOCompressor
    {
        ...
        [DllImport(LzoDll32Bit, EntryPoint = "lzo1x_999_compress", CallingConvention = CallingConvention.Cdecl)]
        ...
        private byte[] _workMemory = new byte[816384L * 4];
        ...
        public byte[] Compress(byte[] src)
        {
            ...
            int outlen = 0;
            if (Is64Bit())
                lzo1x_999_compress(src, src.Length, dst, ref outlen, _workMemory);
            else
                lzo1x_999_compress32(src, src.Length, dst, ref outlen, _workMemory);
        ...
        {
            if (src.Length == dstlen) //for some reason there are also uncompressible chunks which just need to be added to the others
                return src;
            else
            {
                if (_traceSwitch.TraceVerbose)
